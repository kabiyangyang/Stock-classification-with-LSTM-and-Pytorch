...
def get_trading_day_list(start_time, end_time, frequency=None):
    ...
    if isinstance(frequency, int):
        ...
        return trade_days
    df = get_index_price('000300', start_date=start_time, end_date=end_time)
...
# a = get_trading_day_list('2020-01-01', '2020-02-01', frequency=None)
...
def get_periods_index_stocks(index, start_date, end_date=False):
    ...
    if end_date:
        myquery = ({"ticker":  index, "effDate": {"$gte": start_date, "$lte": end_date}})
        mydoc = mycol.find(myquery)
        data = pd.DataFrame(list(mydoc))
        data = data["consID"].unique().tolist()
    else:
        myquery = ({"ticker":  index, "effDate": {"$gte": date_delay([start_date],40)[0], "$lte": start_date}})
        mydoc = mycol.find(myquery)
        data = pd.DataFrame(list(mydoc))
        data = data[data['effDate'] == data['effDate'].iloc[-1]]
        data = data["consID"].unique().tolist()
    return data

# a = get_periods_index_stocks('000300', '2020-01-01', end_date=False)


def get_stock_industry(stocks,  date, level ='industryID1', type='sw'):
    """
    获取股票代码
    :param stocks: 股票代码
    :param level: str, 行业代码
        "industryID1": 一级行业
        "industryID2": 二级行业
        "industryID3": 三级行业
    :param type: str,
    ...
    if type=='sw':
        ...
        mycol = mydb["industry_zx"]
    myquery = ({"secID": {"$in": stocks}, "tradeDate":  date})
    mydoc = mycol.find(myquery)
    data = pd.DataFrame(list(mydoc))
    data = pd.Series(data[level].values, index=data["secID"])
    return data

# a= get_stock_industry(['000001.XSHE','000002.XSHE'],'2020-04-29')
...
def neutralize(data, date, market_cap=None, industry_name='industryID1', country_factor=False):
    """
    中性化，使用行业和市值因子中性化
    input：
    ...
    if industry_name is None:
        ...
        data1['se'] = get_stock_industry(stocks, date, level=industry_name, type='sw')
...
def get_index_comprofit(index, date):
    ...
    """
    temp = [float(get_index_price(index, start_date=None, end_date=date[xx], count=1)['closeIndex']) for xx in
            range(0,len(date))]
...
def Close_price(factor):
    ...
    myquery = ({"secID": {"$in": tuple(stocks)}, "tradeDate": {"$in": tuple(timelist)}})
    mydoc = mycol.find(myquery)
    price_data = pd.DataFrame(list(mydoc))

    mycol1 = mydb["daily_limit_price"]
    myquery1 = ({"secID": {"$in": tuple(stocks)}, "tradeDate": {"$in": tuple(timelist)}})
    mydoc1 = mycol1.find(myquery1)
    limit_price_data = pd.DataFrame(list(mydoc1))
    price_data = pd.merge(limit_price_data[['secID', 'tradeDate', 'limitDownPrice']], price_data, on=['secID', 'tradeDate'],
                          how='right')

    mycol2 = mydb["daily_price_weifq"]
    myquery2 = ({"secID": {"$in": tuple(stocks)}, "tradeDate": {"$in": tuple(timelist)}})
    mydoc2 = mycol2.find(myquery2)
    price_data_weifq = pd.DataFrame(list(mydoc2))
    price_data_weifq['actclose'] = price_data_weifq['closePrice']
    price_data = pd.merge(price_data_weifq[['secID', 'tradeDate', 'actclose']], price_data, on=['secID', 'tradeDate'], how='right')

    factor_price = pd.pivot_table(price_data, index="tradeDate", columns="secID", values="closePrice", dropna=False)
    factor_isOpen = pd.pivot_table(price_data, index="tradeDate", columns="secID", values="isOpen", dropna=False)
    factor_limit = pd.pivot_table(price_data, index="tradeDate", columns="secID", values="limitDownPrice", dropna=False)
    factor_act_price = pd.pivot_table(price_data, index="tradeDate", columns="secID", values="actclose", dropna=False)
    factor_limit = factor_act_price - factor_limit
    factor_limit = factor_limit.applymap(lambda x: 1 if x != 0 else np.nan)
    factor_isOpen = factor_isOpen.replace(0, np.nan)
    factor_price = factor_price * factor_isOpen * factor_limit
...
def Market_value(factor,circulating_market=False):
    ...
    factor_market = pd.pivot_table(valuation_data, index="tradeDate", columns="secID", values=market, dropna=False)
    return factor_market


def Group_by(factor,level ='industryID1', type='sw'):
    """
    factor: dataframe
            index 日期
            columns 股票代码
    ...
    if type=='sw':
        ...
        mycol = mydb["industry_zx"]
    myquery = ({"secID": {"$in": tuple(stocks)}, "tradeDate": {"$in": timelist}})
    mydoc = mycol.find(myquery)
    stock_industry = pd.DataFrame(list(mydoc))
    stock_industry = stock_industry.pivot(index="tradeDate", columns="secID", values=level)
